...
import { 
  X, Save, BookOpen, Sparkles, GraduationCap 
} from 'lucide-react';
import { 
...
interface PhDScholarProfileEditModalProps {
  isOpen: boolean;
  onClose: () => void;
  scholar: Researcher;
}

export const PhDScholarProfileEditModal: React.FC<PhDScholarProfileEditModalProps> = ({ isOpen, onClose, scholar }) => {
  const { updateResearcherProfile, addToast } = useApp();

  const [name, setName] = useState(scholar.name || '');
  const [department, setDepartment] = useState(scholar.department || '');
  const [researchArea, setResearchArea] = useState(scholar.researchArea || '');
  const [specialization, setSpecialization] = useState(scholar.specialization || '');
  const [guide, setGuide] = useState(scholar.guide || '');
  const [bio, setBio] = useState(scholar.bio || '');
  const [researchTopics, setResearchTopics] = useState((scholar.researchTopics || scholar.interests || []).join(', '));
  const [vidwanUrl, setVidwanUrl] = useState(scholar.vidwan_profile_url || '');

  // Social Links
  const [linkedin, setLinkedin] = useState(scholar.socialLinks?.linkedin || scholar.linkedin || '');
  const [github, setGithub] = useState(scholar.socialLinks?.github || scholar.github || '');
  const [twitter, setTwitter] = useState(scholar.socialLinks?.twitter || scholar.twitter || '');
  const [leetcode, setLeetcode] = useState(scholar.socialLinks?.leetcode || scholar.leetcode || '');
  const [instagram, setInstagram] = useState(scholar.socialLinks?.instagram || scholar.instagram || '');
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    const cleanedTopics = researchTopics
      .split(',')
    ...
    updateResearcherProfile(scholar.id, {
      name: sanitizeInput(name),
      department: sanitizeInput(department),
      researchArea: sanitizeInput(researchArea),
      specialization: sanitizeInput(specialization),
      guide: sanitizeInput(guide),
      bio: sanitizeInput(bio),
      researchTopics: cleanedTopics,
      interests: cleanedTopics,
      ...
      <div className="bg-white rounded-3xl max-w-2xl w-full p-6 sm:p-8 shadow-warm-2xl border border-campus-border relative my-8">
        ...
        <div className="flex items-center justify-between pb-4 border-b border-campus-border">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-campus-soft-blue text-campus-deep-blue flex items-center justify-center font-bold">
              <GraduationCap className="w-5 h-5" />
            </div>
            <div>
              <h3 className="text-lg font-bold text-campus-deep-blue">Edit PhD Scholar Profile</h3>
              <p className="text-xs text-campus-muted-text">Update doctoral research topics, Vidwan ID & social links</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSave} className="space-y-4 pt-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3.5 py-2.5 rounded-xl border border-campus-border text-xs sm:text-sm focus:border-campus-blue outline-none"
                placeholder="e.g. Computational & Data Sciences"
                required
              />
            </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3.5 py-2.5 rounded-xl border border-campus-border text-xs sm:text-sm focus:border-campus-blue outline-none"
              placeholder="e.g. Decentralized Machine Learning & Secure Aggregation"
            />
          </div>

          <div>
            <label className="block text-xs font-bold text-campus-deep-blue uppercase tracking-wider mb-1">
              Research Topics & Keywords (Comma Separated)
            </label>
            <input
              type="text"
              ...
              className="w-full px-3.5 py-2.5 rounded-xl border border-campus-border text-xs sm:text-sm focus:border-campus-blue outline-none"
              placeholder="e.g. Federated Learning, Differential Privacy, Healthcare AI, Model Compression"
            />
          </div>

          <div>
            <label className="block text-xs font-bold text-campus-deep-blue uppercase tracking-wider mb-1">
              Academic Bio & Research Abstract
            </label>
            <textarea
              value={bio}
              ...
              rows={3}
              className="w-full px-3.5 py-2.5 rounded-xl border border-campus-border text-xs sm:text-sm focus:border-campus-blue outline-none"
              placeholder="Summary of ongoing doctoral research, lab benchmarks, and student collaboration interests..."
            />
          </div>
          ...
          <div className="p-4 rounded-2xl bg-purple-50 border border-purple-200 space-y-1.5">
            ...
            <input
              ...
              placeholder="https://vidwan.inflibnet.ac.in/profile/184201"
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-campus-border">
            ...
            >
              ...
              <span>{isSubmitting ? 'Saving...' : 'Save Scholar Profile'}</span>
            </button>
          </div>
